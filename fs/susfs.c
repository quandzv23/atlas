...
/*
 * Compatibility stub: this Atlas kernel tree has a "manual hook" call site
 * in kernel/reboot.c (SYSCALL_DEFINE4(reboot, ...)) that expects
 * ksu_handle_sys_reboot() to exist, from an older/different KernelSU fork
 * convention. The rsuntk/KernelSU (deprecated/susfs-legacy) branch used
 * here does not implement this hook, so we provide a harmless no-op stub
 * to satisfy the linker. This does not disable or alter any susfs/KernelSU
 * functionality -- it only means the kernel does not run extra logic
 * specifically at the reboot() syscall entry point.
 */
#ifdef CONFIG_KSU
